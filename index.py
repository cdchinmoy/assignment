    ...
    if(os.path.isfile(path_to_json)):
        ...
        with open(path_to_json, 'r', encoding='utf-8') as infile:
            ...
            for filled_data_key, filled_data_value in filled_data.items():
                filled_data_value_arr = filled_data_value.split(".", 1)
                if(len(filled_data_value_arr) < 2):
                    flag = False
                else:    
                    index = filled_data_value_arr[0]
                    if(index.isnumeric()):
                        value = filled_data_value_arr[1]
                        final_data['data'][filled_data_key][index] = [{'value':value}]
                        flag = True
                    else:
                        flag = False
                    ...
                    infile.close()
        if(flag == True):
            with open(path_to_json, 'w', encoding='utf-8') as infile:
                json.dump(final_data, infile)
                infile.close()

                flash('Data has been updated.', 'success')
                return redirect("/")
        else:
            flash('Invalid update format, it should be (ex: 6.Testing, 7.testing again)', 'denger')
            return redirect("/")
